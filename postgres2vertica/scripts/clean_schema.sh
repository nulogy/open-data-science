...
sed -i.bak 's/extensions.hstore/text/g' $SCHEMADIR/schema_clean.sql
sed -i.bak 's/extensions.citext/text/g' $SCHEMADIR/schema_clean.sql
...
sed -i.bak -e 's/\(.*\) \(\btext\b\|\bcharacter varying\b.*\) NOT NULL/\1 \2/' \
      -e 's/\(.*\) \btext\b/\1 varchar(4096)/' $SCHEMADIR/schema_clean.sql
sed -i.bak 's/::text/::varchar/g' $SCHEMADIR/schema_clean.sql
...
cp $SCHEMADIR/schema_clean.sql $SCHEMADIR/schema_final.sql
